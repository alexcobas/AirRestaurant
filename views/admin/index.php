...
document.getElementById('create-user-option').addEventListener('click', function() {
    document.getElementById('new-user-row').style.display = 'table-row';
});
document.getElementById('create-user-btn').addEventListener('click', createUser);
document.getElementById('users-table').addEventListener('click', deleteUser);
        ...
        if (data.status === 'success') {
            ...
            data.data.forEach(user => {
                ...
                row.innerHTML = `
                    ...
                    <td>
                        <button class="delete-btn btn btn-danger" data-id="${user.id}">Eliminar</button>
                    </td>
                `;
        ...
        if (data.status === 'success') {
            ...
            data.data.forEach(product => {
                ...
                row.innerHTML = `
                    ...
                    <td>${category.name}</td>
                    <td><img src="${imageSrc}" alt="Image" style="width: 50px; height: 50px;"></td>
                    <td>
                        <button class="delete-btn btn btn-danger" data-id="${product.id}">Eliminar</button>
                    </td>
                `;
...
function fetchPedidos() {
    fetch('http://localhost/airrestaurant/api/getPedidos', {
        method: 'GET',
        headers: {
            'Content-Type': 'application/json',
            'Accept': 'application/json',
